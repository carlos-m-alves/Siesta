...
<c:choose>
 <c:when test="${! empty logado}">  
        ...
        <div class="row align-items-center">
          ...
          <nav class="mx-auto site-navigation">
            <ul class="site-menu js-clone-nav d-none d-xl-block ml-0 pl-0">
              <li class="active nav-link"><a href="QuartoServlet?action=pagination&idHospedagem=${idHospedagem}&recordsPerPage=5&currentPage=1" class="nav-link">Lista de Quartos</a></li>
            </ul>
          </nav>
        ...
        <div class="row mb-5 justify-content-center">
          <div class="col-md-7 text-center">
            <h2 class="section-title mb-2">
                ${qtdQuartos} 
                 ${qtdQuartos > 1 ? 'Quartos cadastrados' : 'Quarto cadastrado'}</h2>
          </div>
          <div class="col-md-2">
            <a href="QuartoServlet?action=add" class="btn btn-block btn-primary btn-md">Adicionar</a>
          </div>
        </div>
          
        <div class="mb-5">
            <form id="formDel">
                <input type="hidden" id="idQuarto" name="idQuarto" />
            </form>
            <c:forEach var="quarto" items="${listaQuartos}">   
                <div class="row align-items-start job-item border-bottom pb-3 mb-3 pt-3">
                  <div class="col-md-2">
                    <a href="QuartoServlet?action=inf&idQuarto=${quarto.id}"><img src="images/featured-listing-1.jpg" alt="Image" class="img-fluid"></a>
                  </div>
                  <div class="col-md-4">
                    <h2><a href="QuartoServlet?action=inf&idQuarto=${quarto.id}">${quarto.nome}</a> </h2>
                  </div>
                  ...
                  <div class="col-md-3 text-md-right">
                    <a onclick="mostraModal('Gostaria de excluir definitivamente?', 'CONFIRME',${quarto.id})" class="btn btn-danger btn-md">Excluir</a>  
                    <!--<a href="QuartoServlet?action=delQuarto&idQuarto=${quarto.id}" class="btn btn-danger btn-md">Excluir</a>  -->
                  </div>
                </div>
            </c:forEach>
  ...
  <script>
    function enviaForm(){
        document.getElementById("formDel").action="./QuartoServlet?action=delQuarto";
        document.getElementById("formDel").method = "POST";
        document.getElementById("formDel").idQuarto.value = document.getElementById("idQuarto").value;
    ...
    function mostraModal(msg, func, idParc){      
        ...
        if( func == 'WAIT'){
            ...
            document.getElementById("idQuarto").value = idParc;              
        }else{
            document.getElementById("modalTagConfirme").style.visibility = 'hidden';   
            document.getElementById("modalTagA").style.visibility = 'visible';          
        }
