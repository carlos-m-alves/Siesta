...
<c:choose>
 <c:when test="${! empty logado}">  
        ...
        <div class="row align-items-center">
          ...
          <nav class="mx-auto site-navigation">
            <ul class="site-menu js-clone-nav d-none d-xl-block ml-0 pl-0">
              ...
              <li><a href="AdministradorServlet?action=pagination&recordsPerPage=5&currentPage=1" class="">Lista de Hospedagens</a></li>
              <li><a href="#" class="active">Lista de Parceiros</a></li>
            </ul>
          </nav>
        ...
        <div class="row align-items-center justify-content-center">
          <div class="col-md-12">
            <div class="mb-5 text-center">
              <h1 class="text-white font-weight-bold">Lista de Parceiros</h1>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="row mb-5 justify-content-center">
          <div class="col-md-7 text-center">
            <h2 class="section-title mb-2">
                ${qtdParceiros}
                ${qtdParceiros > 1 ? 'Parceiros cadastrados' : 'Parceiro cadastrado'}</h2>
          </div>
          <div class="col-md-2">
            <a href="AdministradorServlet?action=addFunc" class="btn btn-block btn-primary btn-md">Adicionar</a>
          </div>
        </div>
          
        <div class="mb-5">
            <form id="formDel">
                <input type="hidden" id="idParceiro" name="idParceiro" />
            </form>
            <c:forEach var="parceiro" items="${listaParceiros}">   
                <div class="row align-items-start job-item border-bottom pb-3 mb-3 pt-3">
                  <div class="col-md-2">
                    <a href="AdministradorServlet?action=infFunc&idFunc=${parceiro.id}"><img src="images/featured-listing-1.jpg" alt="Image" class="img-fluid"></a>
                  </div>
                  <div class="col-md-4">
                    <h2><a href="AdministradorServlet?action=infFunc&idFunc=${parceiro.id}">${parceiro.nome}</a> </h2>                    
                  </div>
                  <div class="col-md-3 text-left">
                    <h3>${parceiro.email}</h3>
                  </div>
                  <div class="col-md-3 text-md-right">
                    <a onclick="mostraModal('Gostaria de excluir definitivamente?', 'CONFIRME',${parceiro.id})" class="btn btn-danger btn-md">Excluir</a>  
                    <!-- <a href="AdministradorServlet?action=delParceiro&idParceiro=${parceiro.id}" class="btn btn-danger btn-md">Excluir</a>   -->                    
                  </div>                  
                </div>
            </c:forEach>
          ...
          <div class="col-md-6 text-center text-md-left">
            <div class="custom-pagination ml-auto">
              ...
              <c:if test="${currentPage != 1}">
                <a class="prev" 
                    href="AdministradorServlet?action=paginationFunc&recordsPerPage=${recordsPerPage}&currentPage=${currentPage-1}">
                    Anterior
                </a>
              </c:if>
              ...
              <div class="d-inline-block">
                <c:forEach begin="1" end="${noOfPages}" var="i">
                    <c:choose>
                        ...
                        <c:otherwise>
                            <a class="" 
                                href="AdministradorServlet?action=paginationFunc&recordsPerPage=${recordsPerPage}&currentPage=${i}">${i}</a>
                        </c:otherwise>
                    </c:choose>
                </c:forEach>
              </div>  
  ...
  <script>
    function enviaForm(){
        document.getElementById("formDel").action="./AdministradorServlet?action=delParceiro";
        document.getElementById("formDel").method = "POST";
        document.getElementById("formDel").idParceiro.value = document.getElementById("idParceiro").value;
    ...
    function mostraModal(msg, func, idParc){      
        ...
        if( func == 'WAIT'){
            ...
            document.getElementById("idParceiro").value = idParc; 
            //console.log("id setado: "+document.getElementById("idParceiro").value);
        }else{
            document.getElementById("modalTagConfirme").style.visibility = 'hidden';   
            document.getElementById("modalTagA").style.visibility = 'visible';          
        }
